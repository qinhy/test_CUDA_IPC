...
class Iox2StreamInfo(ctypes.Structure):
...
class Iox2FrameReady(ctypes.Structure):
    """Fixed-size iceoryx2 payload announcing one completed frame."""
    ...
    def __str__(self) -> str:
        return (
            f"Iox2FrameReady(stream={decode_fixed_bytes(self.stream_name)!r}, "
...
def make_stream_info_payload(info: StreamInfo) -> Iox2StreamInfo:
...
def parse_stream_info_payload(obj: Iox2StreamInfo) -> StreamInfo:
...
def make_frame_ready_payload(frame: FrameReady) -> Iox2FrameReady:
    obj = Iox2FrameReady()
    obj.magic = IOX2_MAGIC
    obj.version = IOX2_VERSION
...
def parse_frame_ready_payload(obj: Iox2FrameReady) -> FrameReady:
    if bytes(obj.magic) != IOX2_MAGIC:
