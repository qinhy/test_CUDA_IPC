...
```bat
uv run TCPiceoryx2\producers\video_decode_ipc_publisher.py --ip 127.0.0.1 --streams rgb
```
...
```bat
uv run TCPiceoryx2\consumers\ipc_gl_viewer_pycuda.py --stream rgb
```

### 4. Optional Torch AI consumer

```bat
uv run TCPiceoryx2\consumers\ipc_ai_consumer_torch.py --stream rgb --latest-only
```

## Run all streams

```bat
uv run TCPiceoryx2\tools\tcp_sim_src.py --streams rgb,left,right
uv run TCPiceoryx2\producers\video_decode_ipc_publisher.py --ip 127.0.0.1 --streams rgb,left,right
uv run TCPiceoryx2\consumers\ipc_gl_viewer_pycuda.py --stream rgb
uv run TCPiceoryx2\consumers\ipc_gl_viewer_pycuda.py --stream left
uv run TCPiceoryx2\consumers\ipc_gl_viewer_pycuda.py --stream right
```
