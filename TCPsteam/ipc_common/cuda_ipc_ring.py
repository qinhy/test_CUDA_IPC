...
def normalize_cupy_chw_uint8(a):
    if a.dtype != cp.uint8:
        a = a.astype(cp.uint8)
    ...
    if a.ndim == 2:
        ...
        if a.shape[0] in (1, 3, 4):
            ...
            raise RuntimeError(f"cannot infer channel dimension from shape={a.shape}")
    else:
        raise RuntimeError(f"unexpected CuPy frame shape={a.shape}")

    if x.shape[0] not in (1, 3, 4):
        raise RuntimeError(f"expected 1, 3, or 4 channels, got shape={x.shape}")
